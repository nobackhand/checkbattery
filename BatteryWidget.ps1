...
# --- Load pure estimation math (EMA smoothing + time-from-rate) ---
# The WinForms-free estimation functions (Update-EMARate, Get-CapacityDerivedRate,
# Get-SmoothedTimeRemaining) and their $script: state initializers live in
# BatteryEstimation.ps1 so they can be unit-tested in isolation.
#  - From source: this guarded dot-source loads them (the file sits next to this script).
...
# --- EMA smoothing / hysteresis / cross-validation state ---
# These $script: state variables ($emaRate, $lastValidRate, $lastValidRateTime,
# $rateHistory, $lastCapacityCheck, $capacityRateMismatchCount, $lastAcState,
# $stateChangeTime, $hysteresisSeconds) are now declared in BatteryEstimation.ps1
# alongside the pure estimation functions that own them. They are made available
# by the guarded dot-source near the top of this script (from source) or by the
# build-time inline concatenation (compiled exe). See the dot-source block above.
...
# ============================================================
# EMA SMOOTHING FOR DISCHARGE RATE  (pure math — extracted)
# ============================================================
# The pure estimation functions Update-EMARate, Get-CapacityDerivedRate, and
# Get-SmoothedTimeRemaining (plus their $script: state) now live in
# BatteryEstimation.ps1 so they can be unit-tested in isolation. They are loaded
# by the guarded dot-source near the top of this script when running from source,
# and inlined ahead of this file by Build.ps1 when compiled into the exe.
# No behavior or signatures changed — only the location of the code.
