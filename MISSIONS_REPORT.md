...
- [x] 6. Strengthen typing in the weakest-typed module — no untyped public signatures, stricter checker settings than before. Proof: type checker exits 0. — **DONE** f19d45c. Module: `BatteryWidget.ps1` (49 of the repo's 64 functions, 8 untyped params, **0** declared return types; the only module the exe compiles from). Orchestrator: `tools/check-types.ps1` **exit 0** (13 files / 64 functions / 92 params), `verify.sh` **exit 0**. Stricter = (a) a type checker now exists and is check 7 of the lint gate — every function+script param needs an explicit constraint, every function needs `[OutputType]`, `[object]` requires an `# any-typed:` justification, `[OutputType([void])]` may not return a value; (b) PSScriptAnalyzer `Severity` raised to include `Information`, activating `PSUseOutputTypeCorrectly` (inert before) — the 9 findings were fixed, no rule excluded. Orchestrator negative control: stripped `[int]` from `Format-Duration` → **exit 1** naming the file:line.
- [x] 7. Make verify.sh at least 30% faster without deleting coverage. Proof: before/after timings in the commit message. — **DONE** b4161e4. Orchestrator measured both itself (old script restored from `c74ed1f` and run *in place*, since it resolves repo root from its own location): **old 16889/18325/15390 ms (mean 16.9s) → new 9549/8498/8099 ms (mean 8.7s) = 48% faster** (agent reported 42.9% on its own 5-pair A/B; both clear the 30% bar). Change is in `scripts/verify.sh` only: the three stages are genuinely independent (lint+tests read only; the exe the build writes is no one's input), so they now run as concurrent jobs with output buffered and replayed in the original order — transcript unchanged, only the clock. `VERIFY_JOBS=1` restores serial; concurrency auto-disabled during first-run module bootstrap to avoid a gallery race. Coverage identical: **39 PASS lines before and after**. Orchestrator negative controls in parallel mode: failing test → **exit 1**, lint violation → **exit 1**. Critical path is now lint alone (~8s).
