...
### 2026-07-29 — Typing gate (mission 6)
- **`BatteryWidget.ps1` was the weakest-typed module** — 49 of the repo's 64 functions, and not one of them declared a return type; 8 parameters had no type at all. All 49 now declare `[OutputType(...)]`, and every parameter is typed: `$Now` went from `[object]` to `[Nullable[datetime]]` (3 functions), `Set-DarkMenuItem -Item` to `[ToolStripItem]`, `Add-CenterLabel`'s five parameters to `[string]/[int]/[double]/[bool]/[Color]`. `Read-ConfigField`'s two genuinely polymorphic parameters are explicit `[object]` with an `# any-typed:` justification.
- **New `tools\check-types.ps1`** (see Dev Tools) wired into `check-source.ps1` — the first checker in this repo that looks at types at all.
- **Stricter analyzer settings** — `PSScriptAnalyzerSettings.psd1` `Severity` raised from `Error, Warning` to `Error, Warning, Information`, which turns on `PSUseOutputTypeCorrectly` (return-type rule) and `PSAvoidUsingPositionalParameters` (9 findings fixed in `tests\Appearance.Tests.ps1`).
- Verified: `check-types.ps1` exit 0 (13 files, 64 functions, 92 parameters); each of the 4 rules proven to fail on an injected regression (`missions-evidence/mission-06-negative-control.txt`); `verify.sh` exit 0; render-states 8/8; exe launches ALIVE.
